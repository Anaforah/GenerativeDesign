...
if (cam.available()) {
  ...
  // velocidade de "envelhecimento" das ondas escala com o caudal:
  // caudal "normal" (80 m3/s) = velocidade atual (1x).
  // caudal mais alto -> ondas mais "frenéticas" (mais rápidas)
  float waveSpeed = getDischargeSpeedMultiplier();
  for (int i=0; i<MAX_WAVES; i++) {
    WaveEvent w = waves[i];
    if (w != null) {
      w.age += 0.05 * waveSpeed;
      if (w.age > 20) {
        waves[i] = null;
      }
    }
  ...
  // gerar novas gotas: taxa escala com a intensidade da chuva (0 .. ~2.5/frame)
  if (rainIntensity > 0) {
    float spawnRate = rainIntensity * 2.5;
    int spawnCount = int(spawnRate);
    if (random(1) < (spawnRate - spawnCount)) spawnCount++;
    for (int s = 0; s < spawnCount; s++) {
      rainDrops.add(new RainDrop(rainIntensity));
    }
  }

  // atualizar, desenhar e remover gotas que já saíram pela base
  ...
  // limite de segurança (chuva muito forte / pausas longas)
  while (rainDrops.size() > 250) {
    rainDrops.remove(0);
  }
